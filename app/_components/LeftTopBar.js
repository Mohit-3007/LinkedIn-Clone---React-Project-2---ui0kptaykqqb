...
// getting the first leter of UserName
  ...
  return (
    <>
        {/* image */}
        <div className='w-full h-fit px-3 pt-3 pb-4 border-b-[1px] border-[#E8E8E8] relative'>
        ...
        <Link href={"#"} className={'res-768:block w-full h-fit p-3 border-b-[1px] border-[#E8E8E8] ' + (showMore ? '' : 'hidden')}>
          ...
          <span className='w-full h-4 flex mt-1'>
            ...
            <span onClick={handleAlert} className='w-fit h-full text-xs font-semibold text-[#3F3F3F] hover:text-[#0A66C2] hover:underline'>Retry Premium for &#8377; 0</span>
          </span>
        </Link>

        {/* saved items */}
        <Link onClick={handleAlert} href={"#"} className={'res-768:block w-full h-10 p-3 ' + (showMore ? '' : 'hidden')}>
