...
const GroupDotOption = ({setCheckLocal, groupId}) => {
  ...
  return (
    <div className='w-10 h-10 relative'>
      {/* dots */}
      <div ref={dotRef} onClick={()=> setShowOpt(!showOpt)} className='w-full h-full flex justify-center cursor-pointer items-center text-[#666666] 
      rounded-[50%] hover:bg-[#E8E8E8] dark:text-[rgb(255,255,255,0.6)] dark:hover:bg-[#2C2F33]'>
        <BsThreeDots className='w-6 h-6' />
      </div>
      {/* option initiatlly hidden */}
      <div ref={optionRef} className={'w-[12.0625rem] h-fit py-1 dark:bg-[rgb(27,31,35)] bg-white rounded-lg shadow-xl z-20 absolute right-0 -bottom-24 ' + (showOpt ? "block" : "hidden")}>
        <ul className='w-full h-fit list-none text-sm font-semibold dark:text-[rgb(255,255,255,0.6)] text-[#666666]'>
          <li onClick={copyToClipboard} className='w-full h-10 rounded-sm dark:hover:bg-[#2C2F33] hover:bg-[#E8E8E8] cursor-pointer'>
            <div className='w-full h-full px-4 py-2 flex items-center'>
              <IoIosLink className='w-6 h-6 mr-1 dark:text-[rgb(255,255,255,0.6)] text-[#666666]' />Copy link to group
            </div>
          </li>
          <li onClick={() => handleDeleteGroup(groupId)} className='w-full h-10 rounded-sm dark:hover:bg-[#2C2F33] hover:bg-[#E8E8E8] cursor-pointer'>
            <div className='w-full h-full px-4 py-2 flex items-center'>
              <RxExit className='w-6 h-6 mr-1 dark:text-[rgb(255,255,255,0.6)] text-[#666666]' />Leave this Group
            </div>
          </li>
        </ul>
      </div>
