...
const UpdatePassword = () => {
  ...
  const { userName, userEmail, token } = useContextProvider();
  ...
  return (
    <div className="w-[100vw] h-screen flex flex-col overflow-x-hidden dark:bg-black bg-[#F3F2F0]">
      {/* main */}
      <main className='w-full res-1128:w-[1128px] res-1128:mx-[calc((100%-1128px)/2)] px-4 py-6 h-[calc(100vh-50px)] ' >
        <header className='w-full h-max'>
          ...
          {/* Title */}
          <h1 className='w-full h-max px-4 pt-[14px] dark:text-[rgb(255,255,255,0.9)] pb-6 text-[#1E1E61] text-[32px] text-center'>
            Change your current password
          </h1>
        </header>
        {/* SignUp form wrapper */}
        <div className='w-[400.5px] mx-[calc((100%-400.5px)/2)] h-fit dark:bg-[rgb(27,31,35)] bg-white shadow-lg rounded-md'>
          <p className='w-full px-6 h-[48px] pt-4 pb-3 text-sm dark:text-[rgb(255,255,255,0.9)] text-[#191919] font-semibold'>
          ...
          <form className='w-full h-fit '>
            <section className='w-full h-fit px-6 pt-2'>
                ...
                <div className='w-full h-fit flex flex-col'>
                  {/* Current pass */}
                  <label htmlFor='pass' className='h-5 dark:text-[rgb(255,255,255,0.6)] text-[#666666] text-sm mb-1 block'>Type your current password*</label>
                  <input id='pass' type='password' value={password} 
                    onFocus={() => setIsPassEmpty(false)}
                    placeholder='Current password'
                    onChange={(e) => setPassword(e.target.value)} 
                    className='text-[#191919] dark:text-[rgb(255,255,255,0.6)] dark:focus:outline-[rgb(233,233,234)] focus:outline-[#404040] mb-3 outline-1
                     hover:outline-2 outline outline-[#404040] px-4 text-sm w-full h-[30px] rounded-sm dark:placeholder:text-[rgb(255,255,255,0.6)] dark:outline-[rgb(255,255,255,0.6)] '/>
                  {/* error */}
                  {isPassEmpty && (
                  ...
                  {/* New pass */}
                  <label htmlFor='newPass' className='h-5 dark:text-[rgb(255,255,255,0.6)] text-[#666666] text-sm mt-4 mb-1 block'>Type your new password*</label>
                  <div className='relative w-full h-fit mb-3'>
                    <input ref={newPassRef} id='newPass' type='password' value={newPassword} 
                      ...
                      className={'w-full h-[30px] px-4 rounded-sm text-sm outline outline-1 hover:outline-2 dark:text-[rgb(255,255,255,0.6)] dark:focus:outline-[rgb(233,233,234)] dark:placeholder:text-[rgb(255,255,255,0.6)] dark:outline-[rgb(255,255,255,0.6)]  ' 
                  ...
                  {/* Retype new pass */}
                  <label htmlFor='password' className='h-5 dark:text-[rgb(255,255,255,0.6)] text-[#666666] text-sm mt-4 mb-1 block '>Retype your new password*</label>
                  <div className='relative w-full h-fit mb-3'>
                ...
                <button onClick={handleSubmit} 
                  ...
                  className='w-full h-12 py-3 mt-4 mb-7 px-6 rounded-[24px] disabled:cursor-not-allowed dark:disabled:bg-[#383D40]
                   dark:bg-[#70B5F9] disabled:bg-[#EBEBEB] hover:bg-[#004182] bg-[#0A66C2] dark:text-[rgb(0,0,0,0.75)] disabled:text-[#A4A4A4] text-white text-center text-base font-semibold '>
                  Save Password 
                </button>
            </section>
          </form>
      ...
      {/* footer */}
      <footer className=' w-full h-fit res-992:h-[50px] dark:bg-[rgb(27,31,35)] bg-white'>
        <ul className='w-full px-4 res-992:px-0 py-6 res-992:py-0 flex-wrap res-992:flex-nowrap res-992:w-[1128px] 
          res-1128:w-[1128px] h-fit res-992:mr-[calc(100%-1128px)] text-[#666666] dark:text-[rgb(255,255,255,0.6)] res-1128:mx-[calc((100%-1128px)/2)] list-none flex'>
          <li className='w-[50%] res-992:w-fit h-[34px] p-2 text-sm flex items-center cursor-pointer hover:underline hover:text-[#0a66c2]'>
            <Image src={linke} alt='logo' width={56} height={14} className='w-[56px] h-[14px] mr-2' />
            <span className='w-fit '>&copy; 2024</span>
          </li>
          <li className='w-[50%] res-992:w-fit h-[34px] p-2 text-sm cursor-pointer hover:underline hover:text-[#0a66c2]'>
